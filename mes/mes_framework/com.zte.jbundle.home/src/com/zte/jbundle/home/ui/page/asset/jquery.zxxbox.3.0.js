...
(function($) {
		...
		'<div class="wrap_out" id="wrapOut">' +
			'<div class="wrap_in" id="wrapIn">' +
				'<div id="wrapBar" class="wrap_bar" onselectstart="return false;">' +
					...
					'<div class="wrap_close"><a href="javasctipt:" id="wrapClose" title="Close"></a></div>' +	
				'</div>' +
	...
	$.extend($.zxxbox, {
		...
		//预载
		loading: function() {
			var element = $('<div class="wrap_remind">Loading...</div>');
			$.zxxbox(element, { bar: false });
		},
		//ask询问方法
		ask: function(message, sureCall, cancelCall, options) {
			var element = $('<div class="wrap_remind">'+message+'<p><button id="zxxSureBtn" class="submit_btn">Confirm</button>&nbsp;&nbsp;<button id="zxxCancelBtn" class="cancel_btn">取消</button></p></div>');
			$.zxxbox(element, options);
			//回调方法
		...
		remind: function(message, callback, options) {
			var element = $('<div class="wrap_remind">'+message+'<p><button id="zxxSubmitBtn" class="submit_btn">Confirm</button</p></div>');
			$.zxxbox(element, options);
		...
		ajax: function(uri, params, options) {
			if (uri) {
				...
				$.ajax({
					...
					error: function() {
						$.zxxbox.remind("Loading error.");	//加载出了点问题
					}
				});	
			}
		}
